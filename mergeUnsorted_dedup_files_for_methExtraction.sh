...
for LIB in ${LIST}; do
    ...
    echo -e "\nMerging of bam files completed...\n"
...
for LIB in ${LIST}; do
	...
	for bam_file in $(/bin/ls $BAM_PATH/*_unsorted_merged.deduplicated.bam); do
        $BISMARK_PATH/bismark_methylation_extractor --multicore 2 -p --no_overlap -o $METH_OUTPUT_DIR --comprehensive --report --bedGraph --genome_folder $GENOME_PATH $bam_file
