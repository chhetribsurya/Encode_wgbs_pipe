...
export INPUT_LOC="/gpfs/gpfs1/home/schhetri/wgbs_run/wgbs_split_VIIIth_batch/raw_fastq"

### Search these SL# or library numbers containing all the wgbs fastqs under myerslab data repository:
#export LIB_LIST='SL259540 SL259541 SL259542 SL259543'
#export LIB_LIST='SL259532 SL259532_combined SL259533 SL259534 SL259535 SL259536 SL259537 SL259538 SL259539 SL259540 SL259541 SL259542 SL259543 SL259544 SL259545 SL259546 SL259547 SL259548 SL259549 SL259550 SL259551 SL259552 SL259553 SL259554 SL259555 SL259556 SL259557 SL259558 SL259559 SL259560'
export LIB_LIST='SL273169 SL273170 SL273171 SL273172 SL273173 SL273174 SL273175 SL273176 SL273177 SL273178 SL273179 SL273180 SL273181 SL273182'

### Set the main output dir location to retain all the splitted fastq files:
export OUTPUT_LOC="/gpfs/gpfs1/home/schhetri/wgbs_run/wgbs_split_VIIIth_batch/wgbs_pe"
...
export BISMARK_PATH="/gpfs/gpfs2/software/bismark-0.14.3"
...
### Bsub parameters:
export BSUB_MEM_OPTIONS="-We 24:00 -q c7normal -n 6 -R span[hosts=1] -R rusage[mem=61440]"  
export BSUB_OPTIONS_FOR_COV="-We 24:00 -q c7normal -R rusage[mem=6146]"  
export BSUB_OPTIONS="-We 40:00 -q c7normal -n $CORE_NUM -R span[hosts=1] -R rusage[mem=16384]"  
export BSUB_DEFAULT_OPTIONS="-We 10:00 -q c7normal"  

export JOB_PREFIX="batch_VIII"
...
#bsub -We 24:00 -q c7normal -J "Fastq splitting" -o $LOG_FILES/${JOB_PREFIX}_fastq_split_main.out $RUN_PATH/call_fastq_split.sh
...
#bsub -We 24:00 -q c7normal -J "Bismark and trim galore run for paired-end fastq" -o $LOG_FILES/${JOB_PREFIX}_bismarkrun_main.out $RUN_PATH/call_trim_galore_bismark_alignment.sh  
...
bsub -We 24:00 -q c7normal -J "Deduplication and methylation calling" -o $LOG_FILES/${JOB_PREFIX}_deduplication_methylationCall_main.out $RUN_PATH/call_mergeUnsorted_dedup_files_for_methExtraction.sh

### This job calls the script call_sort_for_coverage.sh, which further calls the sort_for_coverage.sh: 
#bsub -We 24:00 -q c7normal -J "Coverage metrics" -o $LOG_FILES/${JOB_PREFIX}_coverage_metrics_main.out $RUN_PATH/call_sort_for_coverage.sh

### This job calls the script python_bismark_qc_analysis.py for the QC analysis: 
#bsub -We 24:00 -q c7normal -J "Bismark QC analysis" -o $LOG_FILES/${JOB_PREFIX}_python_QC_analysis.out "python $RUN_PATH/python_bismark_qc_analysis.py $ASSAY_TYPE $OUTPUT_DIR" 
...
#bsub -We 24:00 -q c7normal -J "QC analysis for Insert size" -o $LOG_FILES/${JOB_PREFIX}_insert_size_main.out $RUN_PATH/call_insert_size_plots_using_samtools.sh
